...
def create_grid(puzzle: str) -> tp.List[tp.List[str]]:
    """Cоздание grid в нужном формате"""
...
def get_col(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    ['3', '6', '9']
    """
    return [el[pos[1]] for el in grid]
...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    return [
        grid[row][col]
        for row in range(pos[0] - pos[0] % 3, pos[0] - pos[0] % 3 + 3)
        for col in range(pos[1] - pos[1] % 3, pos[1] - pos[1] % 3 + 3)
    ]
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    all_values -= set(get_row(grid, pos))
    all_values -= set(get_col(grid, pos))
    all_values -= set(get_block(grid, pos))
    return all_values
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    for el in "123456789":
        if el in find_possible_values(grid, pos):
            grid[pos[0]][pos[1]] = el
            if solve(grid):
                return grid
            grid[pos[0]][pos[1]] = "."
